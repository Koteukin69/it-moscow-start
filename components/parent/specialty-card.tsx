...
export default function SpecialtyCard({specialtyData}: {specialtyData: SpecialtyData}) {
        ...
        <Card className={`w-full cursor-pointer overflow-hidden ${styles.innerSquare}`}>
          <div className="relative">
            <SpecialtyCardVisual
              ...
              imageAlign="right"
              alt={specialtyData.title}
              iconNames={specialtyData.icons}
              orbPreset={specialtyData.orb}
            />
            <div className="absolute inset-x-0 bottom-0 z-30 bg-gradient-to-t from-black/90 via-black/50 to-transparent px-4 pb-4 pt-10">
              ...
              <p className="text-sm font-semibold leading-snug text-white">{specialtyData.title}</p>
            </div>
          </div>
        </Card>
      </div>
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="flex flex-col gap-5 p-6">
              <div className="flex flex-col gap-1.5">
                <p className="text-sm text-muted-foreground">{specialtyData.description}</p>
                <p className="text-sm text-muted-foreground">{specialtyData.relevance}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="flex flex-col gap-2.5">
                  ...
                  <ul className="flex flex-col gap-2">
                    {specialtyData.targetAudience.map((item, i) => {
                ...
                <div className="flex flex-col gap-2.5">
                  ...
                  <ul className="flex flex-col gap-2">
                    {specialtyData.curriculum.map((item, i) => {
              ...
              <div className="flex flex-col gap-2.5 pb-1">
                ...
                <div className="flex flex-wrap gap-2">
                  {specialtyData.careers.map((career) => (
